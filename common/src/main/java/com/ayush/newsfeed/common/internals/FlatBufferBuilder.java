...
package com.ayush.newsfeed.common.internals;
...
import static com.ayush.newsfeed.common.internals.Constants.FILE_IDENTIFIER_LENGTH;
import static com.ayush.newsfeed.common.internals.Constants.SIZEOF_INT;
import static com.ayush.newsfeed.common.internals.Constants.SIZEOF_SHORT;
...
public class FlatBufferBuilder {
    ...
    public FlatBufferBuilder(int initial_size) {
        ...
        space = initial_size;
        bb = newByteBuffer(initial_size, false);
    }
     ...
     * @return Returns `this`.
     */
    public FlatBufferBuilder init() {

        if (bb == null)
            return new FlatBufferBuilder();
     ...
     */
    static ByteBuffer newByteBuffer(int capacity, boolean direct) {

        final ByteBuffer newbb;
        if (direct)
            newbb = ByteBuffer.allocateDirect(capacity);
        else
            newbb = ByteBuffer.allocate(capacity);
    ...
    static ByteBuffer growByteBuffer(ByteBuffer bb) {
        ...
        bb.position(0);
        ByteBuffer nbb = newByteBuffer(new_buf_size, bb.isDirect());
     ...
     */
    public int createString(char[] s) {
    ...
    public int endObject() {
        ...
        if (existing_vtable != 0) {
            ...
            if (num_vtables == vtables.length)
                vtables = Arrays.copyOf(vtables, num_vtables * 2);
    ...
    /**
     * A utility function to write out the flat buffer data to an OutputStream
     */
    private void writeTo(int start, int length, OutputStream stream) throws IOException {
    ...
    public void writeTo(OutputStream stream) throws IOException {
        writeTo(space, bb.capacity() - space, stream);
    }
}
